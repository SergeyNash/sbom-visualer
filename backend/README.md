...
- **Извлечение архивов** - поддержка ZIP, TAR, TAR.GZ, TGZ, GZ
- **Фильтрация компонентов** - по типу, лицензии, уровню риска
- **REST API** - полнофункциональный API для интеграции

### SPDX (MVP)
- Используются `packages` и `relationships`
- Компоненты нормализуются в `SbomComponent`
- Учитываются связи `DEPENDS_ON`, `CONTAINS` и обратные зависимости (`*_DEPENDENCY_OF`)
