...
SBOM Visualizer построен как **full-stack** приложение:
- **Frontend**: React + TypeScript (UI/визуализация)
- **Backend**: .NET 8 Web API (основная бизнес-логика)

При этом frontend работает в режиме **dual-mode**:
- **Primary**: вызовы в backend через `src/services/apiService.ts`
- **Fallback**: локальная обработка в браузере через `src/utils/*` (режим Offline или при сетевых ошибках в Auto)

## Диаграмма архитектуры

```
┌──────────────────────┐        ┌───────────────────────────┐
│  React Frontend      │  HTTP  │  .NET Backend API         │
│  (UI + Visualization)│◄──────►│  (Parsing/Gen/Merge/ZIP)   │
│                      │        │                           │
│  ┌────────────────┐  │        │  ┌─────────────────────┐  │
│  │ Components     │  │        │  │ Services            │  │
│  │ - Table/Tree   │  │        │  │ - SbomParser        │  │
│  │ - Filters      │  │        │  │ - SbomGenerator     │  │
│  │ - Uploaders    │  │        │  │ - SbomMerger        │  │
│  └────────────────┘  │        │  │ - ArchiveExtractor  │  │
│  ┌────────────────┐  │        │  └─────────────────────┘  │
│  │ sbomOperations │  │        └───────────────────────────┘
│  │ (API+fallback) │
...
- `services/sbomOperations.ts` - единый слой операций (API primary + fallback)
- `services/apiService.ts` - HTTP клиент к backend
- `utils/*` - офлайн fallback (парсинг/мердж/генерация/архивы)

**Ответственность:**
- Управление состоянием приложения
- Выбор источника данных (API vs fallback)
- Алгоритмы анализа данных (в backend или локально)
...
```
User Upload → sbomOperations (API primary) → State Update → UI Update
     ↓                 ↓                         ↓           ↓
  File Input → apiService.uploadSbom() → setSbomData → Re-render
     ↓
  (fallback: validateSBOMFile + parseSBOMFile)
```
...
### Текущее состояние
- ✅ **Unit/Component тесты (frontend)**: есть (Vitest + Testing Library) в `src/__tests__` и `src/**/__tests__`
- ⚠️ **Integration тесты**: частично (в основном на уровне React компонентов/сценариев)
