...
- Извлечение архивов (ZIP, TAR, TAR.GZ, TGZ, GZ)
...
- Dual-mode: **API по умолчанию + offline fallback** (переключается селектором *Mode* в шапке приложения)
- Offline fallback ограничен: парсинг/мердж/генерация выполняются в браузере, архивы — только ZIP
...
│   │   └── sbomOperations.ts  # Unified operations (API primary + local fallback)
│   ├── types/                 # TypeScript типы
│   ├── utils/                 # Offline fallback (legacy) утилиты
│   ├── data/                  # Mock данные
