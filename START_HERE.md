...
- ✅ Новый API клиент
- ✅ Dual-mode: API по умолчанию + Offline fallback
...
**A:** Нет. `src/utils/` остаются как **офлайн fallback** (если backend недоступен) и для обратной совместимости. В обычном режиме приложение использует .NET API.

### Q: Какие форматы архивов поддерживаются?
**A:** В API-режиме поддерживаются ZIP, TAR, TAR.GZ, TGZ, GZ. В offline fallback (браузерный режим) поддерживается только ZIP.

### Q: Можно ли запустить только Frontend?
**A:** Да, но с ограничениями. По умолчанию UI использует backend, однако можно переключить режим на **Offline** (селектор *Mode* в шапке приложения) — тогда часть функций работает локально в браузере.
