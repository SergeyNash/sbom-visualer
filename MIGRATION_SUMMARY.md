...
- **Frontend (React + TypeScript)**: UI + визуализация, **dual-mode** (API по умолчанию + offline fallback)
- **Backend (.NET 8.0 Web API)**: Основная бизнес-логика (парсинг/генерация/слияние/архивы) через REST API
...
```
src/components/
├── SBOMUploader.tsx                 ✅ API primary + offline fallback
└── CodeUploader.tsx                 ✅ API primary + offline fallback
```

### Legacy/Offline fallback (используются при offline режиме или проблемах сети)
```
src/utils/
├── sbomParser.ts                    ⚠️ Fallback парсинг в браузере
├── sbomGenerator.ts                 ⚠️ Fallback генерация в браузере
├── sbomMerger.ts                    ⚠️ Fallback merge в браузере
└── archiveExtractor.ts              ⚠️ Fallback архивы в браузере (ограниченная поддержка)
```
